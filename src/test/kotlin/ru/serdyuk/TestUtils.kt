...
fun chessArguments(path: String): MutableList<Arguments> {
    var nr = 0
    val result: MutableList<Arguments> = ArrayList()
    while (true) {
...
fun chessArgumentsFen(path: String): MutableList<Arguments> {
    var nr = 0
    val result: MutableList<Arguments> = ArrayList()
    while (true) {
        val inFile = Path.of(String.format("%s/test.%d.in", path, nr))
        ...
        val input = Files.readAllLines(inFile)[0]
        val expected = Files.readAllLines(outFile)
